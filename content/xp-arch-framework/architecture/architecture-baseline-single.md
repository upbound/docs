...
The UXP pod and individual providers offer [configuration options]({{<ref "#uxp-and-provider-configuration-options" >}}) to reduce CPU load, at the cost of slower startup and recovery times.
...
For a complete guide to integrating secrets management in your control plane, see the [Interface Integrations > Secrets Management](../../interface-integrations/secrets-management) topic in this framework.
...
An effective way to supplement Crossplane is to enforce governance through policies. Any Kubernetes-compatible policy engine–-such as [Open Policy Agent](https://github.com/open-policy-agent/opa) or [Kyverno](https://github.com/kyverno/kyverno)–-can be used with Crossplane. This allows users to write custom policies to enforce against Crossplane resources. 
...
For a complete guide to integrating policies in your control plane, see the [Interface Integrations > Policy Engines](../../interface-integrations/policy-engines) topic in this framework.
...
To enable metric emission, configure the Helm chart at install time:

```bash
metrics.enabled: true
```

This will enable Prometheus path, port and scrape annotations and expose port 8080 for both the Crossplane and RBAC Manager pods.

For a complete guide to monitoring & observability in your control plane see the [Interface Integrations > Monitoring & Observability](../../interface-integrations/monitoring-and-observability) topic in this framework.

### Platform continuity

To maintain business continuity, define the Service Level Agreement for your control plane. An effective tool to help you achieve your control plane SLA commitment is with [Velero](https://velero.io/). Velero allows users to capture and backup the state of their infrastructure’s configuration on their control plane. In a disaster scenario–such as if the control plane were to go offline–users can provision a new instance of Crossplane and restore the last known state up to the time of the most recent backup. 

For a complete guide to monitoring & observability in your control plane see the [Platform Continuity](../../interface-integrations/platform-continuity) topic in this framework.
...
Your control plane’s API can be consumed in a variety of ways. For users who are building an Internal Developer Platform (IDP), this typically involves having a UI-based form experiences to collect information to send along to a control plane to action upon. If you read and follow the instructions for [building APIs with compositions](../../building-apis/building-apis/compositions). this means you need to create Crossplane claims on the control plane.
