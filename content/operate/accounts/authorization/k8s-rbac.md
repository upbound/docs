...
This guide explains how to authorize actions on resources in your control planes on Upbound. It uses the built-in [Kubernetes role-based access control](https://kubernetes.io/docs/reference/access-authn-authz/rbac/) (RBAC) mechanism. This gives you fine-grained control over control plane access.
...
{{<expand "Show example of RBAC manager roles" >}}
```sh
...
Connect to the control plane and configure it. For this example, you can install [configuration-azure-data](https://marketplace.upbound.io/configurations/upbound/configuration-azure-database):
...
```
Use the [Upbound Console]({{<ref "operate/accounts/identity-management/teams/#add-users" >}}) to add the desired users to the team.
...
You can also use the [Upbound Console]({{<ref "operate/accounts/authorization/upbound-rbac/#assign-group-role-permissions" >}}) to assign this role.
