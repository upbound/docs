...
**Upbound Managed Spaces** is a deployment mode that combines the control and
security of a self-hosted environment with the operational ease of a managed
service. With Managed Spaces, you deploy Upbound's platform in your own cloud
account while granting Upbound's operations team the access needed to maintain
your environment.

A Managed Space is a single-tenant deployment of Upbound into your own AWS,
Azure, or GCP account. Managed Spaces are for organizations that require
...
autonomy. With Managed Spaces, you can use the same API, CLI, and Console that
Upbound offers, with the benefit of running entirely in a cloud account that you
own and Upbound manages for you.

{{< hint "tip" >}}
If you would rather Upbound deploy the Space in Upbound's own cloud
account and manage the infrastructure, that is available with [Dedicated Spaces]({{< ref
"deploy/dedicated-spaces" >}}). {{< /hint >}}
...
{{<img src="/deploy/dedicated-spaces/images/managed-arch-aws.png" alt="Upbound Managed Spaces arch" unBlur="true">}}
...
If you have an interest in Upbound Managed Spaces, contact
[Upbound](https://www.upbound.io/support/contact). We can chat more about your
requirements and see if Managed Spaces are a good fit for you.
