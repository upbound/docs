...
To optimize readability and comprehension Upbound has developed guidelines
...
Dynamic highlighting highlights a specific line when a read hovers over a specific word outside of the code block.
...
First, apply the {{<hover label="example" line="1" >}}{label=some_name}{{</hover >}} to the code fence.
...
Next, use the `hover` shortcode around the word triggering the highlighting. 
Provide the matching `label` name and `line` number to highlight
