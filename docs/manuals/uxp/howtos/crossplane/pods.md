...
Background on the components installed with Crossplane and their functions.
...
[init containers][k8s-init-containers].
:::
...
:::note
The [Install Crossplane][install-crossplane] section has more
...
[changing the Crossplane pod setting](#change-pod-settings)
and enabling the `--enable-realtime-compositions` argument.
...
:::tip
The [Crossplane install guide][install-crossplane-feature-flags]
describes enabling feature flags like `--enable-realtime-compositions`
with Helm.
:::
...
Read the [Change Pod Settings](#change-pod-settings) section for
...
The [RBAC manager design document][rbac-design-doc] 
...
[Crossplane Install][install-crossplane] section. 
:::
...
Use the [ClusterRoles](#crossplane-clusterroles) to grant access to all Crossplane resources in the
cluster.  
...
The full list of [configuration options][install-crossplane-customize] 
and [feature flags][install-crossplane-feature-flags]
are available in the [Crossplane Install][install-crossplane] 
section. 
...
Add Crossplane pod arguments to the `spec.template.spec.containers[].args`
