---
title: Overview 
sidebar_position: 1
description: Spaces are a host environment in Upbound for control planes-as-a-service
---

Upbound Spaces are a core component of the Upbound platform. A _Space_ is a hosting environment for Upbound control planes-as-a-service. Spaces delivers a managed experience for deploying, operating, and scaling control planes powered by [Upbound Crossplane][uxp]. 

Spaces offers [supporting features][features] to help you operate control planes at scale. 

You can choose to run in a [Cloud Space][cloud-spaces], which offers a true SaaS experience, or deploy your own [Self-hosted Space][self-hosted-space]. **These options are available for users on a Standard, Enterprise, or Business Critical plan.**

## Get Started

Read the guides to get started using control planes in Spaces:

- [Cloud Space][cloud-space-guide] guide
- [Self-Hosted Space][self-hosted-space-guide] guide

[uxp]: /manuals/uxp/overview
[cloud-spaces]: /manuals/spaces/guides/cloud-spaces/overview
[self-hosted-space]: /manuals/spaces/guides/self-hosted/overview
[features]: /manuals/spaces/features/managed-service
[cloud-space-guide]: /manuals/spaces/guides/cloud-spaces/overview
[self-hosted-space-guide]: /manuals/spaces/guides/self-hosted/overview