---
title: Release Notes
validation:
  type: reference
  owner: docs@upbound.io
  tags:
    - reference
    - release-notes
---

Upbound's release notes:


- [MCP Connector][mcp-release-notes]
- [Spaces][spaces-release-notes]
- [Up CLI][cli-release-notes]
- [Upbound Crossplane (UXP)][uxp-release-notes]


[cli-release-notes]: /reference/release-notes/up-cli/
[spaces-release-notes]:/reference/release-notes/spaces/
[mcp-release-notes]: /reference/release-notes/mcp-connector/
[uxp-release-notes]: /reference/release-notes/uxp/
