...
In this tutorial, you learn how to create a control plane in Upbound and connect it to [Argo CD][argo-cd]. Argo CD is a continuous delivery tool for Kubernetes that you can use to deliver configurations to control planes in Upbound.
...
- An [account in Upbound][account-in-upbound].
<!-- vale Upbound.Spelling = NO -->
- Have already deployed an instance of Argo CD on a Kubernetes cluster (or using a hosted service such as [Akuity][akuity]).
<!-- vale Upbound.Spelling = YES -->
- The [up CLI][up-cli] on your local machine.
<!-- vale Upbound.Spelling = YES -->
...
<TabItem value="Upbound Console" label="Upbound Console">
...
<!-- ![Create control plane in Console](/deploy/spaces/console-ctp-create.jpeg) -->
</TabItem>
...
You need a personal access token (PAT) to generate a kubeconfig. You create PATs on a per-user basis in the Upbound Console. Go to [My Account - API tokens][my-account-api-tokens] and select `Create New Token`. Give the token a name and save the secret value to somewhere safe.
...
<!-- [up-cli]: /operate/cli/#install-the-up-command-line -->
<!--- TODO(tr0njavolta): links --->
[argo-cd]: https://argoproj.github.io/cd/
[account-in-upbound]: https://accounts.upbound.io/register
